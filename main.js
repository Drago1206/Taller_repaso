        ...
        for(let id in data[dataInput.scale]){
            if(data[dataInput.scale][id][0] <= dataInput.nota && data[dataInput.scale][id][1] >= dataInput.nota){
                ...
                myTabla.insertAdjacentHTML("beforeend",`
                    ...
                    <tr>
                        <td colspan="5">${(id == 0) ? "Querido usuario se gano un viaje a <b>Cancún</b>" : (id == 1) ? "Querido usuario se gano unas voletas para Cine Colombia para ver la pelicula de <b>One pice</b>" : ""}</td>
                    </tr>
                `);
            }
        }
